...
export class ShiftsController {
  ...
  open(@Body() dto: OpenShiftDto, @CurrentUser() user: AuthenticatedUser) {
    return this.shiftsService.open(user.userId, dto, user.storeId);
  }

  @Get('open/me')
  @ApiOperation({ summary: 'الوردية المفتوحة حالياً للمستخدم الحالي' })
  findOpen(@CurrentUser() user: AuthenticatedUser) {
    return this.shiftsService.findOpenByCashier(user.userId, user.storeId);
  }
  ...
  getSummary(@Param('shiftId') shiftId: string, @CurrentUser() user: AuthenticatedUser) {
    return this.shiftsService.getSummary(shiftId, { userId: user.userId, role: user.role as UserRole, storeId: user.storeId });
  }
  ...
  close(@Param('shiftId') shiftId: string, @Body() dto: CloseShiftDto, @CurrentUser() user: AuthenticatedUser) {
    return this.shiftsService.close(shiftId, dto, { userId: user.userId, role: user.role as UserRole, storeId: user.storeId });
  }
}
